"""
Test Script for BERT Issue Type Classifier
...
def test_bert_classifier():
    """Test the trained BERT classifier"""
    
    logger.info("=" * 80)
    logger.info("TESTING BERT ISSUE TYPE CLASSIFIER")
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.error("❌ No trained BERT model found!")
        logger.error("Please train the model first:")
        logger.error("  python train_bert.py --model distilbert-base-uncased")
        return
    
    # Test cases (same as XGBoost for comparison)
    test_issues = [
        {
    ...
    actual_accuracy = actual_correct / 10
    logger.info("\n" + "=" * 80)
    logger.info("ACTUAL ISSUES RESULTS")
    logger.info("=" * 80)
    logger.info(f"Correct: {actual_correct}/10")
    logger.info(f"Accuracy: {actual_accuracy:.2%}")
    
    logger.info("\n" + "=" * 80)
    logger.info("TESTING COMPLETE!")
    logger.info("=" * 80)
    logger.info(f"\nOverall Performance:")
    logger.info(f"  Predefined Cases: {accuracy:.2%}")
    logger.info(f"  Actual Issues: {actual_accuracy:.2%}")
    logger.info(f"  Combined: {(correct + actual_correct)/20:.2%}")
    logger.info("=" * 80)
